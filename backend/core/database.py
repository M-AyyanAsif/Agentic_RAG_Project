...
class DatabaseManager:
    ...
    def get_detailed_sessions(self) -> list[dict[str, str]]:
        """Returns session IDs paired with their first user-entered prompt text as the title preview."""
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT h1.session_id, h1.content as title
